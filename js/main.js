...
function restore(json){
  ...
  parts=[];used.clear();
  SKIP_CUTS=true;                             // 先用沒側孔的幾何把場景撐起來（快）
  try{
    s.parts.forEach(d=>{
  ...
  SKIP_CUTS=true;                             // syncPinFills 也走無側孔的快版
  try{ layout();refresh(); } finally { SKIP_CUTS=false; }
...
function renderDocs(){
  ...
  if(window.syncHowto)syncHowto(a.length>0);
...
(function(){
  ...
  window.syncHowto=function(hasProjects){
    const saved=read();
...
/* ── 給作者建議 ── */
// 到 https://web3forms.com 用你的 Gmail 免費申請，把拿到的 Access Key 貼在這裡。
// 信箱本身不會出現在原始碼裡，只有這把 key。
